...
async function main() {
  ...
  const KalamangStorage =
    await ethers.getContractFactory("KalamangStorage");
  const storageContract = await KalamangStorage.deploy(
    ethers.ZeroAddress,
    feeContract.target,
    kycBitkubChainAddress,
    sdkTransferRouterAddress
  );
  console.log("KalamangStorage deployed at:", storageContract.target);

  const KalamangController = await ethers.getContractFactory(
    "KalamangController"
  );
  const controllerContract = await KalamangController.deploy(
  ...
  const tx = await storageContract.setKalamangController(
